...
public class MessagesPage extends BasePage {
    ...
    private static final By lblDraftMsg = By.xpath("//div[text()='"+MessagingConstants.DRAFT+"']/ancestor::div[2]/div[1]");
    ...
    private static final By lblImportantKW = By.xpath("//div[contains(@class,'text-center')]/b");
    private static final By lblImportantMsg = By.xpath("//div[contains(@class,'text-center')]");
    private static final By btnSearchMessages = By.xpath("//img[contains(@srcset,'Fsearch')]");
    ...
    private static By lblUniqueMsg(String subject) {
        return By.xpath("//span[text()='" + subject + "']/ancestor::div[contains(@class,'flex-col h-full')]/div[2]/div[1]//div[contains(@class,'break-words')]");
    }
...
//            }
    ...
    public void validateImportantMsg(String importantMsg) {
        addToReport("----------Start of validation of important message----------", Status.PASS, false);

        //Validate the phrase "Important:"
        String actualImportantLabel = getTextFromElement(lblImportantKW);
        if (MessagingConstants.MESSAGES_IMPORTANT_LABEL.equals(actualImportantLabel)) {
            addToReport("Keyword '" + MessagingConstants.MESSAGES_IMPORTANT_LABEL + "' is present", Status.PASS, true);
        } else {
            addToReport(
                    "Keyword validation failed. Expected: '" + MessagingConstants.MESSAGES_IMPORTANT_LABEL +
                            "', Found: '" + actualImportantLabel + "'",
                    Status.FAIL, true);
        }

        //Validate the remaining message
        String actualImportantMsg = getTextFromElement(lblImportantMsg);
        if (actualImportantMsg.contains(importantMsg)) {
            addToReport("Important message validated successfully: '" + importantMsg + "'", Status.PASS, true);
        } else {
            addToReport(
                    "Important message validation failed. Expected: '" + importantMsg +
                            "', Found: '" + actualImportantMsg + "'",
                    Status.FAIL, true);
        }


        addToReport("----------End of validation of important message----------", Status.PASS, false);
    }
    /**
...
//            clickOnElement(latestMessage);
    ...
    public void inboxMessagesValidation(String subject,String messageID,String message,String deletionSuccessMsg,String recoverySuccessMsg) {
        ...
        try {
            ...
            if (messageID.equals(getTextFromElement(lblUniqueMsg(subject)))) {
                addToReport("Received message subject is :"+messageID, Status.PASS,true);
            } else {
                addToReport("Received message subject is not loaded", Status.FAIL);
            }
            ...
            waitForElementToBeInvisible(imgGreyLoader,LONG_WAIT);
            waitForElementToBeClickable(btnDeleteMessage(1),LONG_WAIT);
            ...
            clickOnElement(btnSearchMessages);
            waitForElementToBeInvisible(imgGreyLoader,LONG_WAIT);
            waitForElementToBeInvisible(lblCustomLoader,LONG_WAIT);
            ...
            String actualMessageID = getTextFromElement(lblUniqueMsg(subject));

            if (messageID.equals(actualMessageID)) {
                addToReport("Deleted message is validated: '" + messageID + "'", Status.PASS, true);
            } else {
                addToReport(
                        "Deleted message validation failed.\nExpected (message ID): '" + messageID + "'\nFound: '" + actualMessageID + "'",
                        Status.FAIL, true );
                throw new RuntimeException("Deleted message is not loaded correctly. Expected: '" + messageID + "', but found: '" + actualMessageID + "'");
            }
//
//            String a = getTextFromElement(lblUniqueMsg(subject));
//            if (messageID.equals(getTextFromElement(lblUniqueMsg(subject)))) {
//                addToReport("Deleted message is validated"+messageID, Status.PASS,true);
//            } else {
//                addToReport("Deleted message is not loaded instead loaded message :"+messageID, Status.FAIL);
//                throw new RuntimeException("Deleted message is not loaded");
//            }
            //Recover the message
            ...
            waitForElementToBeClickable(icnReadMessageRecord(1),LONG_WAIT);
    ...
    public void draftMessageValidations(String subject,String successMsg, String OTP, String messageCreationSuccessMsg,String message,String updatedMsg, String deletionSuccessMsg,String filePath) {
            ...
            //Validate important notice msg
//        if(!getAttributeOrText(getElementByPlaceholder(ElementType.textarea,MessagingConstants.START_TYPING),"value").equals(importantMsg)){
...
//            }
//            waitForElementToBeClickable(getElementByTypeAndText(ElementType.span,MessagingConstants.SAVE_AS_DRAFT),MODERATE_WAIT);
//            clickOnElement(getElementByTypeAndText(ElementType.span,MessagingConstants.SAVE_AS_DRAFT));
            ...
            if(!getTextFromElement(lblDraftMsg).equals(updatedMsg)){
                ...
                addToReport("Validated updated draft message : "+updatedMsg, Status.PASS,true);
            }
