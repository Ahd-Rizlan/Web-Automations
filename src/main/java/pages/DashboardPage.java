...
public class DashboardPage extends BasePage {
    ...
    private static final By lblAccountsOrCards = By.xpath("//h1[contains(text(),'Portfolio')]");
    ...
    private static final By btnCreditCard = By.xpath("//div[contains(@class,'Container_body')]//div[contains(text(),'Credit Cards')]");
    ...
    private static final By imageViewList = By.xpath("//img[contains(@src, '.d59774e7')]");
    private static final By imageDialog = By.xpath("//img[contains(@src, '3D0000000001')]");
    private static final By tblStatement = By.xpath("//table//tr[contains(@class,'rounded-lg')]");
    private static final By lblNickName = By.xpath("(//div[@class='flex']/span)[2]");
    private static final By ddAcountFrom = By.xpath("//select[contains(@id,'accountfrom')]");
    private static final By txtQuickBillPaymentAmount = By.xpath("//input[contains(@inputmode, 'numeric')]");
...
//            clickOnElement(alertPopup);
    ...
    /**
     * Validate all Credit card account details in dashboard
     *
     * @param currencyType - currency type
     */

    public void validateAllCreditCardAtDashboard(String[] currencyType) {
        waitFor(VERY_SHORT_WAIT);
        waitForElementToBeInvisible(lblLoadingIcon, LONG_WAIT);
        waitForElementToBeInvisible(lblAccountListLoading, LONG_WAIT);

        //click button Loans
        if (isElementClickable(btnCreditCard)) {
            clickOnElementUsingJS(btnCreditCard);
        } else {
            addToReport("Unable to find loans button", Status.FAIL);
            throw new RuntimeException("Error - Loans button is not found");
        }

        int totalAccounts = 1; // Default to 1 (no pagination)
        ...
        if (isElementPresentBy(icnAccounts)) {
            ...
            try {
                String[] cardCount = CommonUtils.splitText(getAttributeOrText(icnAccounts, "text"), "/");
                totalAccounts = Integer.parseInt(cardCount[1].trim());
            } catch (Exception e) {
                addToReport("Pagination parsing failed: " + e.getMessage(), Status.FAIL);
                throw e;
            }
        }
        ...
        for (int i = 0; i < totalAccounts; i++) {
            ...
            String cardNumber = getTextFromElement(lblAccountNumber).trim();
            ...
            String can = getTextFromElement(getValueBeforeLabel(CreditCardConstants.CAN_LABEL)).trim();
            if (CommonUtils.containsNumericCharacters(can)) {
                addToReport("Customer Account Number is valid: " + can, Status.PASS, false);
            } else {
                addToReport("Invalid Customer Account Number: " + can, Status.FAIL);
            }

            String expiryDate = getTextFromElement(getValueBeforeLabel(CreditCardConstants.EXPIRY_DATE_LABEL)).trim();
            ...
            String status = getTextFromElement(lblAccountStatuses).trim();
            ...
            String balance = getTextFromElement(lblBalance).trim();
            ...
            String typeLogo = getAttributeOrText(imageSrc, "src");
            ...
            String[] currencySplit = CommonUtils.splitText(balance, " ");
            if (Arrays.asList(currencyType).contains(currencySplit[0])) {
                addToReport("Currency is valid: " + currencySplit[0], Status.PASS, false);
            } else {
                addToReport("Currency format issue: " + currencySplit[0], Status.FAIL);
            }
    ...
    public void selectQuickActions(String quickActionButton) {
        ...
        switch (quickActionButton.toLowerCase()) {
            ...
            case "obtain":
                ...
                break;
            case "web card":
                clickOnElement(btnQActionsWebCard);
                break;
            default:
...
//            }
        ...
        try {
            ...
            if (recordCount == 0) {
                ...
                if (Remarks == null || Remarks.trim().isEmpty()) {
                    addToReport("No remarks found for recent vishwa transactions.", Status.WARNING, true);
                } else if (CommonUtils.containsAlphabaticCharacters(Remarks)) {
                ...
                clickOnElement(btnClosePopup);
                waitForElementToBeInvisible(btnClosePopup, MODERATE_WAIT);
    ...
    public void navigateToMainMenu(String menuName) {
        ...
        waitForElementToBeInvisible(lblLoadingIcon, LONG_WAIT);
        hoverOverElement(btnMainMenu(DashboardConstants.DASHBOARD_UPPERCASE));
    ...
    /**
     * This method will validate the nickName with dashboard card
     */
    public void validateAccountNickName() {

        clickOnElement(lblAccountNumber);

        clickOnElement(imageViewList);
        waitForElementToBeInvisible(lblAccountListLoading, LONG_WAIT);
        waitForElementPresence(tblStatement, LONG_WAIT);

        List<WebElement> rows = driver.findElements(tblStatement);
        HashMap<String, String> accountNumbersAndNickNameValues = new HashMap<>();

        for (int i = 1; i <= rows.size(); i++) {
            String accountNumberDetailView = getTextFromElement(getTransactionTableCell(i, 1)).trim();
            String nicknameDetailView = getTextFromElement(getTransactionTableCell(i, 5)).trim();
            // Add to HashMap
            accountNumbersAndNickNameValues.put(accountNumberDetailView, nicknameDetailView);
        }
        ...
        for (int i = 0; i < totalAccounts; i++) {
            ...
            //  Get card details fresh in each loop
            String cardNumber = CommonUtils.removeSpaceCharacters(getTextFromElement(lblAccountNumber));
            String nickName = CommonUtils.removeSpaceCharacters(getTextFromElement(lblNickName));

            if (accountNumbersAndNickNameValues.containsKey(cardNumber)) {

                String expectedNickName = accountNumbersAndNickNameValues.get(cardNumber);
                boolean bothNoNickName = expectedNickName.equals("-") && (nickName == null || nickName.isEmpty());

                if (bothNoNickName || expectedNickName.equals(nickName)) {
                    addToReport("Card " + cardNumber + " validated with nickname '" + (nickName.isEmpty() ? "No Nickname" : nickName) + "'", Status.PASS, false);
                } else {
                    addToReport("Nickname mismatch for card " + cardNumber +
                            ". Expected: '" + expectedNickName + "', Found: '" + nickName + "'", Status.FAIL, false);
                }
            } else {
                addToReport("Card number " + cardNumber + " not found in table", Status.FAIL, false);
            }
        }
    }

    /**
     * This method will validate the empty payment popUp
     */

    public void PaymentPopUpEmpty() {

        clickOnElement(imageDialog);
        waitForElementPresence(ddAcountFrom, LONG_WAIT);

        sendKeysToElement(txtQuickBillPaymentAmount, Keys.BACK_SPACE, 15);

        String emptyAmount = getTextFromElement(txtQuickBillPaymentAmount).trim();
        if (emptyAmount.isEmpty()) {
            addToReport("After clearing the amount section it's Empty " + emptyAmount, Status.PASS);
        } else {
            addToReport("After clearing the amount section is not Empty " + emptyAmount, Status.FAIL);
        }
        isElementPresentBy((getElementByTypeAndText(DashboardPage.ElementType.button, DashboardConstants.BACK)));
        clickOnElement(getElementByTypeAndText(DashboardPage.ElementType.button, DashboardConstants.BACK));
    }
