...
//    private static final By lblLoans = By.xpath("(//a[@class='SubMenu_subMenuItem___oYCo'])[4]");
    private static final By lblLoans = By.xpath("//a[contains(@class,'SubMenu_subMenuItem')]//div[normalize-space(text())='Loans']");
    ...
    private static final By btnRightTableArrow = By.xpath("//img[contains(@src, '2FArrowRight2')]");


    public static By getLoanDetailValueByLabel(String label) {
        return By.xpath("//li[span[1][normalize-space()='" + label + "']]/span[2]");
    }
    ...
    private static By getSummaryValueByLabel(String label) {
        return By.xpath("//div[contains(@class,'justify-between')][div[normalize-space()='" + label + "']]/div[2]");
    }
    ...
    public void NavigateToLoanPage() {
        ...
        if (waitForElementPresence(getLoanDetailValueByLabel(LoanConstants.OVERDUE_AMOUNT), PawnConstants.WAIT_EXTREME_LONG)) {
    ...
    public void ValidatingLoanDetails(String[] currencyType, String errorMsg) {
        ...
        addToReport("----------Start Validating  the Loan section ----------", Status.INFO, false);
...
// === Final Flag Check ===
        ...
        waitForElementPresence(lblDuration, LONG_WAIT);
        addToReport("Loan duration is visible", Status.PASS, false);

        clickOnElement(btnSettleLoan);
        waitForElementPresence(getSuccessfulMsg(errorMsg), LONG_WAIT);
        ...
        addToReport("----------End Validating  the Loan section ----------", Status.INFO, false);
    ...
    public void ValidatethePartialSettlement(String loanAccountNumber, String accountNumber, String successMsg) {
        ...
        addToReport("----------Start Validating the Loan partial settlement  ----------", Status.INFO, false);
        ...
        waitForElementPresence(btnSubmit, LONG_WAIT);
        ...
        String customAmount = CommonUtils.generateRandomAmountTwoDecimals(0, 100); //generating random amount
        ...
        total = getTextFromElement(getSummaryValueByLabel("Total")).replace("LKR", "").trim();
...
// Check if amount + charges equals total
        ...
        addToReport("----------End Validating the Loan partial settlement  ----------", Status.INFO, false);
    ...
    public void ValidatetheInstallmentDetails() {
        addToReport("----------Start Validating the Loan installment details ----------", Status.INFO, false);
        ...
        waitForElementPresence(tblPartialInstallments, LONG_WAIT);
        addToReport("Partial Installments table visible", Status.PASS);

        boolean matchFound = false;
        boolean hasNextPage = true;
        int pageNumber = 1; // Initialize page number counter

        // Corrected line: Declare fullDate before the loop
        ...
        while (hasNextPage) {
            // Get all rows in the current table page
            List<WebElement> rows = driver.findElements(tblPartialInstallments);

            // Loop through each row on the current page
            for (int i = 1; i <= rows.size(); i++) {
                String paidAmount = getTextFromElement(tblCellRecord(i, 1)).trim();
                String transactionDate = getTextFromElement(tblCellRecord(i, 2)).trim();
                String cleanAmount = paidAmount.replace("LKR", "").replace(",", "").trim();

                if (cleanAmount.matches("\\d{2}-\\d{2}-\\d{4}")) {
                    addToReport("Skipping row: found a date instead of amount in Paid Amount column -> " + cleanAmount, Status.INFO, false);
                    continue;
                }

                try {
                    double parsedTotal = Double.parseDouble(total);
                    double parsedCleanAmount = Double.parseDouble(cleanAmount);

                    boolean amountMatches = parsedTotal == parsedCleanAmount;
                    boolean dateMatches = fullDate.equalsIgnoreCase(transactionDate);

                    if (amountMatches && dateMatches) {
                        // Corrected line: Added page number to the report
                        addToReport("Found matching Paid Amount: '" + paidAmount + "' and Transaction Date: '" + transactionDate + "' on a table row on Page " + pageNumber + ".", Status.PASS, false);
                        matchFound = true;
                        break;
                    }
                } catch (NumberFormatException e) {
                    addToReport("Failed to parse clean amount '" + cleanAmount + "'. Skipping row.", Status.FAIL);
                }
            }

            // If a match was found, we can exit the outer loop
            if (matchFound) {
                break;
            }

            // Check for the "next page" button (right arrow)
            if (isElementPresentBy(btnRightTableArrow)) {
                clickOnElement(btnRightTableArrow);
                waitFor(SHORT_WAIT); // Wait for the page to load
        ...
        if (matchFound) {
            ...
            // Corrected line: Added else block with screenshot
            addToReport("Failed to find Paid Amount '" + total + "' with Date '" + fullDate + "' in the Paid Transaction table after checking all pages.", Status.FAIL);
        }
        ...
        for (int i = 1; i <= rowss.size(); i++) {
            ...
            if (CommonUtils.containsAlphNumAndSpecialCharactersandSpace(capitalDemand)) {
            ...
            if (CommonUtils.containsAlphNumAndSpecialCharactersandSpace(interestDemand)) {
            ...
            if (CommonUtils.containsAlphNumAndSpecialCharactersandSpace(collectionAmount)) {
        ...
        addToReport("----------End Validating the Loan installment details ----------", Status.INFO, false);
    }
