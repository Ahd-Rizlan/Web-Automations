...
public class OpenSavingAccountPage extends BasePage {
    ...
    private static final By btnBack = By.xpath("//button[contains(@class, 'bg-gray-500') and contains(text(), 'Back')]");
    ...
    public void enteringFundDetails(String amount, String nickName) {
        ...
        try {
            ...
            addToReport(" Used '" + amount + "' for the new account", Status.PASS, false);

            clickOnElement(ddPurpose);
            selectFromDropdown(ddPurpose, "0", MessagingConstants.INDEX);
            addToReport("Selected first purpose option:", Status.PASS, false);

            clickOnElement(ddsourceOfFunds);
            selectFromDropdown(ddsourceOfFunds, "0", MessagingConstants.INDEX);
            addToReport("Selected first source Of Funds option:", Status.PASS, false);

            sendKeysToElement(txtnickName, nickName);
            addToReport(" Used '" + nickName + "' for the new account", Status.PASS, false);


            addToReport("----------End of Entering the details----------", Status.INFO, true);
    ...
    /**
     * Validating Saving products
     *
     * @param rawData - saving product details
     */
    public void validateAllSavingsAccounts(List<Object[]> rawData) {

        addToReport("---------- Start the saving account product validation   ----------", Status.INFO, false);

        for (int i = 0; i < SaveAccountConstants.USER_PRODUCTS_SAVING.length; i++) {
            String productName = SaveAccountConstants.USER_PRODUCTS_SAVING[i];
            Object[] row = rawData.get(i);

            String invalidAmount = row[0].toString();
            String validAmount = row[1].toString();
            String expectedError = row[2].toString();
            String successMsg = row[3].toString();

            // Step 1: Select Product
            clickOnElement(ddproductAccount);
            selectFromDropdown(ddproductAccount, productName, "visibleText");
            ...
            if (isElementPresentBy(lblErrorForFunds, SHORT_WAIT)) {
                ...
                if (expectedError.equalsIgnoreCase(actualError)) {
                    addToReport("Correct error message shown: " + actualError, Status.PASS);
                } else {
                    addToReport("Unexpected error message: " + actualError, Status.FAIL);
                }
            }

            // Step 4: Clear amount field
            sendKeysToElement(txtAmount, Keys.BACK_SPACE, 15);

            // Step 5: Enter valid amount
            sendKeysToElement(txtAmount, validAmount);
            addToReport("Entered valid amount: " + validAmount, Status.PASS, false);
            clickOnElement(btnsubmit);
            ...
            if (isElementPresentBy(getSuccessfulMsg(successMsg))) {
                addToReport("Success message shown: " + successMsg, Status.PASS);
            } else {
                addToReport("Expected success message not found!", Status.FAIL);
                throw new RuntimeException("Success message not displayed.");
            }

            // Step 7: Click back and reset
            clickOnElement(btnBack);
            waitForElementPresence(txtnickName);
            sendKeysToElement(txtAmount, Keys.BACK_SPACE, 15);
            addToReport("Returned to account creation page for next product", Status.PASS, false);
        }
        ...
        addToReport("Selected Savings product from dropdown: " + SaveAccountConstants.USER_PRODUCTS_SAVING[1], Status.PASS);
        sendKeysToElement(txtAmount, SaveAccountConstants.AMOUNT);
        addToReport(" Send the Amount for account creation: " + SaveAccountConstants.AMOUNT, Status.PASS, true);

        addToReport("---------- End the saving account product validation   ----------", Status.INFO, false);
    }
    ...
    public void validateSavingConfirmationDetails() {
        ...
        addToReport("---------- Start the saving account confirmation validation   ----------", Status.INFO, false);
        ...
        addToReport("Entered Product: " + expectedProduct, Status.PASS, false);
        ...
        addToReport("Entered Amount: " + expectedAmount, Status.PASS, false);
        ...
        addToReport("Selected Source of Funds: " + expectedSourceOfFunds, Status.PASS, false);


        List<String> purposeList = getSelectedOptionText(ddPurpose, "FIRST_SELECTED");
        String expectedPurpose = purposeList.get(0);
        addToReport("Selected Purpose of Account: " + expectedPurpose, Status.PASS, false);
...
// === Source of Funds ===
        ...
        if (expectedSourceOfFunds.equalsIgnoreCase(actualSourceOfFunds)) {
            addToReport("Source of Funds matched: " + actualSourceOfFunds, Status.PASS, true);
        } else {
            addToReport("Source of Funds mismatch. Expected: " + expectedSourceOfFunds + ", Actual: " + actualSourceOfFunds, Status.FAIL);
        }

        addToReport("---------- End the saving account confirmation validation   ----------", Status.INFO, false);
    }
    ...
    public void enterOTPAndContinueSettingsPage(String otp, String successMsg, String suffixMsg) {
        ...
        try {
            ...
            waitForElementPresence(getPartialSuccessMsg(successMsg), LONG_WAIT); // E.g., "Your new savings account"
    ...
    public String validateSaveAccountConfirmation() {
        try {
            ...
            addToReport("---------- End of validating the confirmation message on the saving page ----------", Status.INFO, false);
