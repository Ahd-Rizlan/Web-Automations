...
public class FDDetailViewPage extends BasePage {
    ...
    private static final By lblFixedDeposits = By.xpath("//a[contains(@class, 'SubMenu_subMenuItem')]//div[normalize-space()='Fixed Deposits']");
    ...
    /**
     * Navigate to the FD detail vide page
     */

    public void NavogatetoFDDetailViewPage() {

        addToReport("----------Start Navigating to the FD Details view section  ----------", Status.INFO, false);
        ...
        addToReport("----------End Navigating to the FD Details view section  ----------", Status.INFO, false);
    }
     ...
     * @param fdAccountNumber - FD account number
     */
    public void validateAllFDAccountsAtDashboard(String[] currencyType, String expectedMessage, String fdAccountNumber) {

        addToReport("----------Start Validating the FD Details view section  ----------", Status.INFO, false);
        ...
        String CurrencyAndAmt = getTextFromElement(lblCurrencyAndAvailableBalance);
        if (CommonUtils.containsAlphNumAndSpecialCharacters(CurrencyAndAmt)) {
            addToReport("Successfully validated currency and amount ", Status.PASS, false);
        } else {
        ...
        if (CurrencyAndAmt.equalsIgnoreCase(depositAmount)) {
            addToReport("Deposit Amount matches: " + depositAmount, Status.PASS, false);
        } else {
            addToReport("Deposit Amount mismatch. Expected: " + CurrencyAndAmt + ", Actual: " + depositAmount, Status.FAIL);
        }
...
//validate the values that only available under the account summary section
        ...
        if (lienAmt[0].equals("0.00")) {
            addToReport("Successfully validated Lien Amount", Status.PASS, false);
        ...
        waitForElementPresence(lblFdDetails,LONG_WAIT);
        addToReport("Successfully visible the detail view", Status.PASS, false);


        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy"); // Update format as UI
...
// Log current system date
        ...
        addToReport("----------End Validating the FD Details view section  ----------", Status.INFO, false);
    }

    /**
     * This method will Validate the error message
     *
