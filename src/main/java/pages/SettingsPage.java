...
//lbl, btn, tf, icn, rdo, chk, img, dd , lnk
    ...
    private static final By lblsetAsPrimaryAccountSpan = By.xpath("//span[text()='Set as ']");
...
public void NavigatetoToOTPVerificationMode (){
    ...
    /**
     * This method use to revert the password change
     *
    ...
    public void revertPasswordToOldValue(String newPassword, String password) {
        try {
            ...
            if (isTileVisible) {
                ...
                addToReport("Password page title is not visible.", Status.FAIL);

            }
            // Logic to reset the password back to the old password
            sendKeysToElement(tfoldPassword, newPassword);
            sendKeysToElement(tfnewwPassword, password);
            sendKeysToElement(tfconfirmPassword, password);
            clickOnElement(btnConfirmSave);
    ...
    public void enterOTPToChangeThePrimaryAccount(String primaryAccountSentSuccessMsg) {
        ...
        try {
            ...
            if (isElementPresentBy(getSuccessfulMsg(primaryAccountSentSuccessMsg))) {
                addToReport("'" + primaryAccountSentSuccessMsg + "' success message is present.", Status.PASS,true);
            } else {
