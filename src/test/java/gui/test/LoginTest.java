...
public class LoginTest extends Drivers {
    ...
    @Test(dataProvider = "LoginPageValidation", description = "Validate the loading of logging page", dataProviderClass = DataProviders.LoginDataProvider.class)
    public void validateTheLoginPage(String loginPageTitle, String loginTileName) {
        loginPage.validateTheLoginPage(loginPageTitle, loginTileName);
    }

    @Test(priority = 1, dataProvider = "LoginData", description = "Validate the successful logging with correct user ID & Password.", dataProviderClass = DataProviders.LoginDataProvider.class)
    public void validateTheSuccessfulLogin(String userName, String password, String dashboardPageTitle, String buttonText) {
        loginPage.loginToSampathVishwaWeb(userName, password, dashboardPageTitle, buttonText);
    }
    ...
    @Test(priority = 3, dataProvider = "IncorrectPassword", description = "Validate the logging with correct user ID & invalid password.", dataProviderClass = DataProviders.LoginDataProvider.class)
    public void validateTheUnSuccessfulLoginWithIncorrectPassword(String userName, String buttonName, String IncorrectPassword, String errorMessageWithAttempts) {
        loginPage.ValidateLoginWithIncorrectPassword(userName, IncorrectPassword, buttonName, errorMessageWithAttempts);
    }
